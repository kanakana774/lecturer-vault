...
> **Fill Mode（フィルモード）の指定で、書式が確保した余白を詰めるという意味です。**
> 空白が見えるように `[` `]` で囲んで比べます。
>
> ```sql
> SELECT
...
> ```
>
> 空白が3つあるのは、`999,999` が7文字に見えて**8文字分**の場所を確保するからです。
>
> | 場所 | 文字数 | `1500` を入れると |
> | :--- | ---: | :--- |
> | 符号 | 1 | 空白（負の数なら `-`） |
...
> | カンマ | 1 | `,` |
>
> **正の数でも符号のために1文字空く**のがつまずきやすいところです。`FM` はこの空白も消します。
> ただし詰まるのは空白だけで、`0` 書式のゼロは残ります（`TO_CHAR(1500, 'FM00000円')` → `01500円`）。
