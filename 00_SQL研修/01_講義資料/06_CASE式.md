...
> [!note]- 書式の頭に付いている `FM` は何か
> **Fill Mode（フィルモード）の指定で、「書式の桁数に合わせて入る余白を詰める」という意味です。**
>
> `TO_CHAR` の数値書式は `9` 1文字が1桁ぶんの場所を確保します。桁が足りない分は**空白で埋められて右寄せ**になり、
> 表示用の文字列としては余分な空白が付いてきます。`FM` を付けると、この空白が入りません。
>
> 空白が見えるように `[` `]` で囲んで比べます。
>
> ```sql
> SELECT
>   '[' || TO_CHAR(1500,  '999,999円')   || ']' AS fmなし,
>   '[' || TO_CHAR(1500,  'FM999,999円') || ']' AS fmあり;
> ```
>
> ```
>     fmなし    |  fmあり
...
> ```
>
> **空白が3つあることに注意してください。** 内訳は次のとおりで、`999,999` は7文字に見えて
> **8文字分の場所**を確保します。
>
> | 場所 | 何文字 | `1500` を入れると |
> | :--- | ---: | :--- |
> | 符号 | 1 | 空白（正の数なので） |
> | `9` 6個ぶんの桁 | 6 | `  1500`（4桁なので余白2つ） |
> | カンマ | 1 | `,` |
>
> **`TO_CHAR` は正の数でも符号のために1文字空けます。** 負の数を入れるとそこが `-` になります。
>
> ```sql
> SELECT
>   '[' || TO_CHAR(1500,   '999,999') || ']' AS 数値4桁,
>   '[' || TO_CHAR(123456, '999,999') || ']' AS 数値6桁,
>   '[' || TO_CHAR(-1500,  '999,999') || ']' AS 負の数;
> ```
>
> ```
>    数値4桁   |  数値6桁   |   負の数
...
> ```
>
> 6桁を入れても空白が1つ残るのが符号の場所です。`FM` はこの空白も消します。
>
> 詰めるのは**空白だけ**です。`9` のかわりに `0` を書いた場所はゼロで埋める指定なので、`FM` を付けても
> ゼロは残ります（`TO_CHAR(1500, 'FM00000円')` → `01500円`）。
>
> 桁をそろえて縦に並べたいなら `FM` を付けない、文字列として使うなら `FM` を付ける、と考えてください。
